	...
	if ((memcmp(&m_pGIFHeader->m_cSignature, "GIF", 3) != 0) &&
		...
		if (nFrameCount == 1) {
			...
			while (hFrameData = GetNextGraphicBlock(&nBlockLen,
				...
				if (CreateStreamOnHGlobal(hFrameData, TRUE, &pInnerStream) != S_OK) {
					GlobalFree(hFrameData);
					continue;
				};

				if (OleLoadPicture(pInnerStream, nBlockLen, FALSE,
					IID_IPicture,
...
BOOL CPictureEx::Draw(CRect* rect) {
	if (!m_bIsInitialized) {
		TRACE(_T("Call one of the CPictureEx::Load() member functions before calling Draw()\n"));
		return FALSE;
	};
	...
	if (IsAnimatedGIF()) {
		...
		if (m_pPicture) {
			long hmWidth;
			long hmHeight;
			//m_pPicture->get_Width(&hmWidth);//Ö±½ÓÓÃrect
			//m_pPicture->get_Height(&hmHeight);
			if (m_pPicture->Render(m_hMemDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy,
				0, rect->Height(), rect->Width(), -(rect->Height()), NULL) == S_OK) {
				Invalidate(FALSE);
				return TRUE;
			};
		};
...
BOOL CPictureEx::Load(LPCTSTR szResourceName, LPCTSTR szResourceType) {
	...
	if (!szResourceName || !szResourceType) {
		OutputDebugString(_T("szResourceName or szResourceType is NULL. Return.\n"));
		return FALSE;
	}
...
int CPictureEx::GetNextBlockLen() const {
	...
	case BLOCK_IMAGE:
		...
		int nInnerTmp = GetSubBlocksLen(m_nCurrOffset +
			sizeof(TGIFImageDescriptor) + nLCTSize + 1);
		if (nInnerTmp > 0)
			return sizeof(TGIFImageDescriptor) + nLCTSize + 1 + nInnerTmp;
		break;
	};
...
UINT WINAPI CPictureEx::_ThreadAnimation(LPVOID pParam) {
	ASSERT(pParam);
	if (!pParam) {
		OutputDebugString(_T("pParam is NULL. Return.\n"));
		return 1;
	}
	...
	if (!pPic) {
		OutputDebugString(_T("pPic is NULL. Return.\n"));
		return 1;
	}
